      ...
      <nav className="bg-blue-900 text-white px-6 py-4 flex justify-between items-center shadow-lg">
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <span className="text-blue-200 text-sm">
            👤 {user.full_name || 'User'} | 🎯 Credits:{' '}
            <strong className="text-white">{user.screening_credits ?? 0}</strong> | 📋 Plan:{' '}
            <strong className="text-yellow-300 uppercase">{user.plan || 'free'}</strong>
          </span>

          <button
            onClick={() => navigate('/pricing')}
            className="bg-yellow-500 hover:bg-yellow-400 px-4 py-2 rounded-lg text-sm font-bold transition-all"
          >
            ⚡ Upgrade
          </button>
      ...
      <div className="max-w-6xl mx-auto p-6">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          ...
          <div className="bg-white rounded-xl p-6 shadow-sm border-l-4 border-green-500">
            ...
            <p className="text-3xl font-bold text-green-700">{user.screening_credits ?? 0}</p>
          </div>

          <div className="bg-white rounded-xl p-6 shadow-sm border-l-4 border-purple-500">
            <p className="text-gray-500 text-sm">Current Plan</p>
            <p className="text-3xl font-bold text-purple-700 uppercase">{user.plan || 'free'}</p>
          </div>
        </div>
                ...
                <div className="flex justify-between items-start">
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-gray-500 text-sm mt-2">
                      👥 {s.total_candidates || 0} candidates
                    </p>
                  </div>
                </div>
              </div>
